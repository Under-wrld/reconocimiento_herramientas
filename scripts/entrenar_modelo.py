...
#Se hace 'activa' uso de GPU y configurar memoria dinámica
...
def generar_reporte_clasificacion(modelo, val_data, clases, salida='static/stats/reporte_clasificacion.csv'):
    val_data.reset()
    pred = modelo.predict(val_data, verbose=0)
...
def entrenar_modelo(data_dir='data/entrenar/', img_size=(224, 224), batch_size=64, epochs=30, metrics=None):
    ...
    model = construir_modelo(num_clases=num_clases, input_shape=(img_size[0], img_size[1], 3), metrics=metrics)
    ...
    print(f"Entrenamiento completo en {duracion:.2f} segundos ({duracion/60:.2f} minutos)")
    ...
    print("Modelo final guardado en modelos/modelo_final.h5")
